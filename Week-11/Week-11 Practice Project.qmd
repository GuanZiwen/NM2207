---
title: "Week-11 Practice Project"
format: html
editor: visual
---
...
historic_state_data_url <- "https://api.covidactnow.org/v2/states.timeseries.json?apiKey=2ffe9447462546af9d9bbe71984e27df" 
...
```{r, eval=T,echo=TRUE}
nrecovered <- data$actuals$cases - data$actuals$deaths

ggplot(data, aes(x=state,y=(nrecovered/population))) + geom_bar(stat="identity") + labs(x="States",y="Recovered (%)")+theme_bw()
```

#iv. What fraction of the population is currently vaccinated ? data$actuals$vaccinationsCompleted
```{r, eval=T,echo=TRUE}

ggplot(data, aes(x=state,y=(data$actuals$vaccinationsCompleted/population))) + geom_bar(stat="identity") + labs(x="States",y="Vaccinated (%)")+theme_bw() 
...
time_series_transmission[seq(1,1300,by=100),]%>% pivot_longer(cols=Alaska:District_of_Columbia,names_to="Countries",values_to="Transmission") %>% ggplot(aes(x=Date,y=Transmission,colour=Countries,group=Countries)) + geom_point(show.legend=TRUE) + labs(x="Date",y="Transmission Level")+theme_bw() 
```
...
fig5 + theme(legend.justification = c(0,1)), align = "v", axis = "lr", nrow=3, ncol = 2,labels = LETTERS[1:5],rel_heights = c(1,2))
```
